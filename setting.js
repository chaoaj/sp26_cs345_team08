...
function drawSettingsMenu() {
  ...
  // Popup image
  image(Game.assets.settingsPopup, POPUP.x, POPUP.y, POPUP.w, POPUP.h);
  ...
  text(Game.isMuted ? 'Muted' : int(Game.sfxVolume * 100) + '%', 960, SFX_SLIDER.y + 10);
  text(Game.isMuted ? 'Muted' : int(Game.musicVolume * 100) + '%', 960, MUSIC_SLIDER.y + 10);
 ...
 /*  fill(240);
  ...
  // Mute button
  if (!Game.isMuted) {
    if (isInsideButton(mouseX, mouseY, MUTE_BTN)) {
      image(Game.assets.settingsMuteButtonHover, MUTE_BTN.x, MUTE_BTN.y, MUTE_BTN.w, MUTE_BTN.h);
    } else {
      image(Game.assets.settingsMuteButton, MUTE_BTN.x, MUTE_BTN.y, MUTE_BTN.w, MUTE_BTN.h);
    }
  } else {
    if (isInsideButton(mouseX, mouseY, MUTE_BTN)) {
      image(Game.assets.settingsUnmuteButtonHover, MUTE_BTN.x, MUTE_BTN.y, MUTE_BTN.w, MUTE_BTN.h);
    } else {
      image(Game.assets.settingsUnmuteButton, MUTE_BTN.x, MUTE_BTN.y, MUTE_BTN.w, MUTE_BTN.h);
    }
  }
  ...
  // Close button
  if (isInsideButton(mouseX, mouseY, CLOSE_BTN)) {
    image(Game.assets.settingsCloseButtonHover, CLOSE_BTN.x, CLOSE_BTN.y, CLOSE_BTN.w, CLOSE_BTN.h);
  } else {
    image(Game.assets.settingsCloseButton, CLOSE_BTN.x, CLOSE_BTN.y, CLOSE_BTN.w, CLOSE_BTN.h);
  }
...
function drawSlider(slider, volume) {
  ...
  // Slider circle image
  image(Game.assets.sliderCircle, handleX, trackY - 14, 25, 25);
}
